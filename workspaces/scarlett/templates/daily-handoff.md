...
**Purpose:** Compose and post the platform digest to #general-engineering: what shipped, what needs eyes, what to watch.
...
You received an `agent.task.request` with `taskType: daily-handoff` to produce the platform update for `#general-engineering`. Your audience is the engineering team starting their day.
