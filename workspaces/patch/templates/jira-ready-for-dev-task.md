...
**Purpose:** Execute the approved engineering-task plan exactly as written, staying pinned to the plan, with tests appropriate to the change, and open green PR(s).
...
BullMQ retries this whole template on failure. Call `jira_get_issue` for `{{ issue.key | default("") }}` with `fields: "status"` first.
...
Pull the latest plan comment, it's the contract. The canonical Task structure is defined in `writing-great-task-issues.md` (included above). The **Acceptance Criteria** and **Definition of Done** are what you ship toward.
