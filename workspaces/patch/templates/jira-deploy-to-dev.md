...
**Purpose:** Merge the human-approved PRs for a ticket into `development` unchanged, as approved, and advance the ticket to Deployed to Development.
...
Push the fix to the PR branch, wait for CI to re-green, then re-evaluate Steps 3–4. Anything beyond a single-line annotation/ignore, such as refactors, test rewrites, dependency bumps, "while I'm here" cleanup, is out of scope at this stage; escalate instead.
