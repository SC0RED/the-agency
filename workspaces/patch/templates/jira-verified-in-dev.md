...
**Purpose:** Mechanically pulse-promote every Verified-in-Development ticket by merging `development` → `testing` across the three repos and advancing those tickets to Deployed to Testing, making no code changes or cleanup along the way.
...
Call `jira_get_issue` for `{{ issue.key | default("") }}` with `fields: "status"`.
...
If the response's `total` is non-zero: **stop**. Call `jira_add_comment` on `{{ issue.key | default("") }}` listing the pending tickets and noting "promotion deferred until those are verified." Do NOT transition the trigger ticket; it stays in Verified-in-Development so the next pulse picks it up. End the run cleanly.
...
If `{{ issue.key | default("") }}` isn't in the response (race? stale read?), escalate to Blocked via `jira_transition_issue` + `jira_add_comment`.
...
2. **Find or create the promotion PR.** Call `github_pr_list` with `state: "open"`, `base: "testing"`, `head: "<owner>:development"`. If response is non-empty, reuse the existing PR. Otherwise call `github_pr_create`:
   ...
   - `head`: `development`
   - `base`: `testing`
   - `title`: `Promote development → testing (pulse: {{ issue.key | default("") }})`
   - `body`: A summary listing the verified-in-dev tickets from Step 3 and the trigger ticket {{ issue.key | default("") }}.

3. **Wait for CI green.** Call `github_pr_check_runs` every ~60s. If any check's `conclusion` is anything but `success`/`skipped`/`neutral`, **stop**: `jira_transition_issue` (Blocked) on `{{ issue.key | default("") }}` + `jira_add_comment` naming the failing repo + PR + check. The other Verified-in-Dev tickets stay in their state; the next pulse retries once the failure is resolved.
...
- Bold paragraph: `Promoted to testing in pulse triggered by {{ issue.key | default("") }}.`
