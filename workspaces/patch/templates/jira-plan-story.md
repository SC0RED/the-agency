...
**Purpose:** Plan a Story: clarify the user need and map the architecture it must fit into, reusing existing patterns, then write an approved-shape plan for Scarlett to review.
...
BullMQ may retry this whole template, so Step 1 can run more than once on the same ticket. Handle every status case explicitly.
...
The Cargo-Cult Patterns, Premature Abstraction, and Time-Optimization Bias guidance in the injected *AI Anti-Patterns* doc above applies to the plan you write here.
