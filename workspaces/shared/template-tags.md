...
The Agency runtime's template engine recognizes two tiers of doc-injection tags. The tier you choose decides whether the injected content participates in Anthropic's prompt cache or pays full input-token rates on every webhook firing.

(Throughout this doc, tags are written in prose form — `system-doc:<path>`, `shared:<path>`, etc. The actual template syntax wraps each one in double mustache braces, the same form Jinja uses; this doc avoids the literal braces because shared docs may be injected into templates and a literal mustache pair inside an injected doc gets parsed by the renderer.)
...
- **No → `doc:` / `shared:`.** Anything that varies per webhook event: the rendered template body itself (Steps, payload table), Jinja expressions referencing the webhook payload (`issue.fields.*`, `payload.*`), memory-recall fragments, anything embedding event-specific values.

When in doubt, run the doc through this filter: open it and search for double-mustache Jinja expressions or any other render-time variables. If the rendered output varies between two `renderTemplate` calls of the same template, it's per-event content and belongs in the body tier.
...
- **Per-event variables inside `system-*` content.** A doc that references render-time Jinja variables (the webhook payload, event metadata) will produce different bytes per run. The cache key changes; you pay the cache-write penalty without the read benefit. Net loss vs leaving it in the body. Keep these in `doc:` / `shared:`.
...
1. **Search the doc for double-mustache expressions.** If it has any literal Jinja expressions referencing payload-derived variables (e.g. `issue.fields.*`, `payload.*` wrapped in mustaches), it's per-event — leave it in `doc:` / `shared:`. The engine renders system content through Jinja too, so unrendered mustache literals inside an injected doc will be evaluated.
...
- **Engine:** the Agency runtime (`SC0RED/Agency`, `src/templates/`) preprocesses `system-*` and `doc:` / `shared:` tags before template rendering. `system-*` content is collected in document order and joined as the run's system prompt; `doc:` / `shared:` content is inlined into the rendered body. The renderer itself is `minijinja`.
- **Runner:** the claude-cli runner (`src/runners/claude_cli.rs`) forwards the joined system prompt via `--append-system-prompt`; when empty, the flag is omitted (templates with no `system-*` tags render byte-identically before and after).
